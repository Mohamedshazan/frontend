...
export default function AssetAgreementPage() {
    ...
    // New: Preview PDF before download
    const handlePreviewPDF = async (assetId: number, assetType: string) => {
        try {
            const response = await api.get(`/users/${id}/assets/${assetId}/agreement-pdf`, {
                responseType: 'blob',
            });
    ...
    const renderAssetRow = (asset: Asset, index: number, isAssigned: boolean) => (
        <tr key={asset.id} className="text-center hover:bg-gray-50 text-gray-800">
            ...
            <td className="p-2 border space-x-2">
                <button
                    onClick={() => handlePreviewPDF(asset.id, asset.asset_type)}
                ...
                <button
                    ...
                    className={`px-3 py-1 rounded text-white ${emailLoading === asset.id ? 'bg-gray-500' : 'bg-blue-600 hover:bg-blue-700'}`}
                >
    ...
    const renderAssetTable = (assets: Asset[], isAssigned: boolean) => (
        <table className="min-w-full border-collapse border border-gray-300 text-sm">
            <thead>
                <tr className="bg-gray-100 text-gray-900">
                    ...
                    <th className="p-2 border">{isAssigned ? 'Assigned Date' : 'Returned Date'}</th>
                    <th className="p-2 border">Actions</th>
                </tr>
            </thead>
            <tbody>
                {assets.map((asset, index) => renderAssetRow(asset, index, isAssigned))}
            </tbody>
        </table>
    );
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto bg-white text-gray-900 rounded shadow-md">
            <h1 className="text-3xl font-bold mb-6 text-green-700">
                Asset Agreement for User ID: {id}
            </h1>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <section className="mb-8">
                        <h2 className="text-xl font-semibold mb-4 text-green-700">Assigned Assets</h2>
                        {assignedAssets.length === 0
                            ? <p className="text-gray-600">No currently assigned assets.</p>
                            : renderAssetTable(assignedAssets, true)}
                    </section>

                    <section>
                        <h2 className="text-xl font-semibold mb-4 text-green-700">Old Assets (Returned)</h2>
                        {oldAssets.length === 0
                            ? <p className="text-gray-600">No old assets found.</p>
                            : renderAssetTable(oldAssets, false)}
                    </section>
                    ...
                    {pdfBlobUrl && (
                        <div className="mt-6">
                            <h3 className="text-xl font-semibold mb-2 text-green-700">
                                PDF Preview - {selectedAssetType} Agreement
                            </h3>
                            <iframe
                                src={pdfBlobUrl}
                                width="100%"
                                ...
                                title="Asset Agreement PDF Preview"
                            />
                        </div>
                    )}
                </>
